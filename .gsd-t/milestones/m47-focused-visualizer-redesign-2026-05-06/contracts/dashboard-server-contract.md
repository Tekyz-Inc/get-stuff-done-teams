...
**Version**: 1.3.0 (M47 D2 — in-session status field + /api/main-session)
...
### GET /api/main-session  (M47 D2)
- **Response**: JSON `{ filename, sessionId, mtimeMs }` for the most-recently-modified `in-session-*.ndjson`, or `{ filename: null, sessionId: null, mtimeMs: null }` when none exists
- **Content-Type**: `application/json`
- **Cache**: `Cache-Control: no-store` (the viewer hits this on every page load)
- **Validation**: filenames must pass `isValidSpawnId` (path-traversal guard); invalid candidates are skipped silently
- **Behavior**:
  1. Scan `transcriptsDir(projectDir)` for files matching `in-session-*.ndjson`
  2. For each candidate `f`, derive `spawnId = f.slice(0, -'.ndjson'.length)` and skip if `!isValidSpawnId(spawnId)`
  3. Return the candidate with the highest `stat.mtimeMs`
  4. `sessionId = f.slice('in-session-'.length, -'.ndjson'.length)`
- **Purpose**: D1 viewer fetches this on `/transcripts` page load to populate the top pane (main conversation stream) with zero clicks. The viewer then opens an SSE stream against `/transcript/in-session-{sessionId}/stream` for live updates.

### In-Session Entry Status Field  (M47 D2)
- `listInSessionTranscripts(projectDir)` returns each entry with a `status` field of `'active' | 'completed'`
- Derivation: `status = (Date.now() - stat.mtimeMs) < 30_000 ? 'active' : 'completed'`
- The 30s window is intentional — the conversation-capture hook fires on every prompt/tool-use turn; a 30s gap means the orchestrator stopped writing
- Boundary semantics are intentionally fuzzy: an entry exactly at the 30s mark may resolve either way per clock skew, both are valid
- Propagates through `handleTranscriptsList` to the merged `/transcripts` JSON response
- **Deliberate non-goals**: `success | failed | killed` taxonomy is out of scope. A future status-source upgrade (orchestrator exit-code wiring) can extend the enum without a viewer change — the M47 D1 viewer code branches on `spawn.status`, so any new value lights up automatically with a neutral badge.
...
module.exports = {
  ...
  listInSessionTranscripts,                    // M45 D2 — in-session NDJSON walk; returns entries with M47 D2 `status` field
  handleMainSession,                           // M47 D2 — GET /api/main-session
