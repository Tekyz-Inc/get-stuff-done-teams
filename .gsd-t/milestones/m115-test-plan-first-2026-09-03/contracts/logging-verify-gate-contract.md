...
- **(0) The declaration file** (`.gsd-t/logging-manifest.json`, 2026-09-03): a project whose streams live where the candidate lists below do not guess DECLARES them — `{ "trace": { "module", "store" }, "audit": { "module", "store", "retention" } }`. A declaration wins over path guessing and is CHECKED, never trusted: a declared module or store path that does not exist FAILs (`logging-manifest-invalid`), a malformed manifest FAILs. `store` is a repo-relative path (inspected like a discovered store) or `{ "kind", "table" }` naming an EXTERNAL store (e.g. Postgres) whose rows cannot be opened offline — that is reported in `notes`, and the module surface (append-only declared, no update/delete path, retention) becomes the enforced evidence. `retention: "indefinite"` declares a never-purged audit log and satisfies `audit-retention-configurable` by policy (noted). Origin: TimeTracking's audit is `server/src/audit.ts` → Postgres `tb_audit_log`, a genuine append-only store the candidate lists could not see. `gsd-t migrate-logging` skips a stream the manifest declares, so declaring an existing audit never scaffolds a second one.
