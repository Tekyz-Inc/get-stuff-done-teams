...
- **Lives INSIDE milestone definition** (`/gsd-t-milestone` + its workflow), at TWO ALTITUDES:
  ...
  2. **Detailed pseudocode** — decomposed to `PseudoCode-{Milestone}.md` at the granularity of the binvoice PayPal doc (per-step intention+mechanism, guard map, divergence flags, appendix). **The milestone is not "DEFINED" until the detailed doc is signed off.**
...
- **Document ripple (user-required):** `PseudoCode-{Milestone}.md` joins the living-documents ripple set — when code/contract/schema changes during the milestone, the pseudocode doc updates in the same pass (Pre-Commit Gate + `commands/gsd-t-doc-ripple.md` + the Living Documents table in `templates/CLAUDE-global.md`). The doc stays the truth, not a stale start-of-milestone snapshot.
