...
## 52. The installer never removes what it stops shipping (files half-fixed, one class of bug, three sightings)
- **Type:** bug | **App:** gsd-t | **Category:** cli
- **Added:** 2026-08-10 | **Priority:** HIGH | **Origin:** conversation 2026-08-09/10 — surfaced three times in one session
- **Problem:** installing only ever COPIES. A file GSD-T stops shipping stays on every machine forever, and only the installer can reach those machines. Three sightings in a single session:
  1. **11 retired commands** (`brainstorm`, `discuss`, `prompt`, `audit`, `reflect`, `optimization-apply/reject`, `unattended` ×3, `visualize`) still typeable months after M38/M61 deleted them. `/gsd-t-brainstorm` was treated as a live command and turned out to have no source at all. **FIXED in v5.11.12** — `removeRetiredCommands()` deletes `gsd-t-*.md` files the package no longer ships.
  2. **26 dead copies of `gsd-t-concise-rewrite.cjs`** left in project `bin/` folders after M107 was retired in v5.11.15. Removed by hand this session — nothing automatic would have.
  3. **A stale `gsd-t-verify-gate.cjs` in all 33 projects** (recorded in the M108 entry, 2026-08-07): present but a different version, so every project ran an outdated quality gate. That one was a stale-CONTENT case rather than a should-be-deleted case, but it is the same root: the installer has no concept of "what I delivered last time."
- **The gap:** commands are now cleaned (v5.11.12). **`bin/` tools, `scripts/` files, and templates are NOT.** A retired tool sits in 32 projects indefinitely; worse, a project-local copy WINS over the package copy in at least one lookup (`findRewriter` preferred `<project>/bin/` over the installed package), so a stale local file can shadow a fixed one — that is how a fixed rewriter kept running the broken version in this session's worktree.
- **Proposed:** generalise `removeRetiredCommands` into one "remove what we no longer ship" pass covering `bin/`, `scripts/`, and templates, for both the global install and every project. Constraints, learned from the command fix:
  - **Only GSD-T-owned names.** The command version matches `gsd-t-*.md` so a user's own file is never considered. The tool version needs the same guard — a project's own `bin/` may hold its own scripts.
  - **NAME every removal.** Silent deletion of a file someone might depend on is worse than the staleness.
  - **HALT on an unreadable directory** rather than reporting a clean pass over a check that never ran.
  - **A manifest of what was delivered** is the sturdier design than "anything matching the prefix that we don't ship now" — the prefix rule cannot tell a retired GSD-T tool from a GSD-T-shaped file the user wrote. Worth deciding which, not assuming.
- **Also worth fixing in the same pass:** the project-copy-wins lookup order. If a project-local copy is allowed to shadow the package, a propagation fix does not actually reach the running code — the exact failure that made a published rewriter fix look broken.
- **Test that proves it:** seed a probe home + probe project with a retired tool, run install, assert the tool is gone, a user's own file survives, and a currently-shipped tool is untouched. Mirrors `test/m112-retired-commands.test.js`.
- See: [[project_global_bin_propagation_gap]] (this is the same family — occurrence 5 was caller-ships-callee-doesn't; this is ships-then-never-unships), and the v5.11.12 + v5.11.15 Decision Log entries.


## 53. Read .env files with values kept out of context (names to the model, values to the process)
- **Type:** feature | **App:** gsd-t | **Category:** security
- **Added:** 2026-09-03
- Origin: the `Read(.env*)` deny rules in `~/.claude/settings.json` were removed on 2026-09-03 because Claude Code 2.1.259 applies them to any Bash command that might touch such a file and deny overrides bypass mode — a permission prompt in every project. That leaves env files readable. David asked: can GSD-T read them while keeping credentials safe? Yes — three deterministic pieces, no deny rule: (1) **redacting read** — PostToolUse on `Read` + shell output: for env files, replace every value with a mask (`KEY=•••• (40 chars, ends …3f)`), reusing the read-intercept hook shape and the heartbeat `scrubSecrets`; (2) **`gsd-t env` command** — `names` (keys + set/unset), `check VAR`, `run -- <cmd>` (loads the file into the child's env so tests get real values that never print) — the M102 `$VAR` principle at runtime; (3) **value-dump guard** — PreToolUse Bash that refuses exact dump patterns (`cat .env*`, `printenv`, `echo $KEY`) pointing at (1)/(2), claude-p-guard shape. Honest limit: a program printing its own config, a stack trace with a connection string, or a committed env file in `git diff` are caught only where the scrubber's patterns match — "values out of context by default," not airtight. Small build: one hook, one command, one guard.


## 54. Demo-video narration: deterministic batch split instead of silence guessing
- **Type:** improvement | **App:** gsd-t | **Category:** demo-videos
- **Added:** 2026-09-03
- `templates/demo-videos/scripts/walkthrough-voice.mjs` renders narration in batches of lines (one TTS call per batch keeps the voice consistent) and then cuts the batch audio into one clip per line by detecting silences. When the cut does not yield exactly N clips it used to fall back to one call per line — which reintroduces the voice drift batching exists to prevent (David 2026-09-03: "will I end up with the video I want? No"). The fallback is now a HALT after three attempts. The real fix: make the split deterministic — a spoken marker or a fixed pause between lines the cutter can find exactly, so the batch never fails to split. Until then, an unsplittable batch stops the render with "rerun or lower BATCH".
