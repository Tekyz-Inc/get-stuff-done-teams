...
|----|----------|----------|
| `branch-guard` | error | Reads project `CLAUDE.md`. Looks for the expected-branch rule in either shape a project writes it: a sentence (`Expected branch: main`, with optional markdown emphasis/backticks) or a two-column table row (`\| Expected branch \| \`main\` \|`). If found, compares to `git branch --show-current`. `ok:false` if mismatch. **Worktrees:** inside a linked worktree (detected via `bin/gsd-t-worktree-detect.cjs` — `--git-dir` ≠ `--git-common-dir`) the rule governs the main checkout only: on a named branch → `ok:true` naming the skip; detached HEAD → `ok:false` (commits would be lost). If no rule is declared, `ok:true` with `msg:"NOT CHECKED — this project declares no expected branch"` (a pass that compared nothing names itself). If `CLAUDE.md` is absent, `ok:true` with `msg:"NOT CHECKED — no CLAUDE.md in this project"`. If `CLAUDE.md` exists but is **unreadable**, `ok:false` — a permissions/corruption failure halts rather than passing as "no rule". |
