...
> **⚠ v2.0.0 — FABLE REMOVED (2026-07-24).** The 4-tier map (haiku/sonnet/opus/fable) collapsed to **3 tiers (haiku/sonnet/opus)** where `opus` = **`claude-opus-5`**. Opus 5 shipped at the same price as Opus 4.8 ($5/$25 per M tokens) but >2× its coding score and within 0.5% of Fable 5 at max effort, so the Fable cost premium ($10/$50 — double Opus 5) is no longer justified. **Every stage formerly on Fable OR Opus 4.8 now runs Opus 5.** The M82 competition judge-blindness invariant is **RELAXED** from "judge model ≠ producers model" to "judge runs in a fresh independent context" — producers AND judge both run `opus`. The `requiresThinkingOmitted` predicate is retained as a single-home no-op (no current model requires omission; the Fable HTTP-400 case it encoded is gone). **Historical references to Fable below are kept for provenance; the LIVE policy is all-opus.**
...
```
requiresThinkingOmitted(model) === true   iff   model === "claude-opus-5"
                                                OR model starts with "claude-opus-5[" 
                                                (the runtime's bracket-suffixed display form, e.g. claude-opus-5[1m] — measured live 2026-06-09)
```

Rationale (the single canonical home): `claude-opus-5` returns **HTTP 400** when the explicit thinking-disabled parameter is sent. The parameter must therefore be OMITTED for Fable. No other file may re-implement this predicate or re-state the rationale.
...
|-----------|-----------------|------|-----------|
| `solution-space-probe`      | gsd-t-phase.workflow.js   | `opus` | M84 |
| `partition-probe`           | gsd-t-phase.workflow.js   | `opus` | M84 |
| `competition-judge`         | gsd-t-phase.workflow.js   | `opus` | M82 — judge is a DIFFERENT model than producers (producers stay opus) |
| `competition-producers`     | gsd-t-phase.workflow.js   | `opus`  | M82 blindness — HELD, do NOT move to fable |
| `pre-mortem`                | gsd-t-phase.workflow.js   | `opus` | M83 |
| `red-team`                  | gsd-t-verify.workflow.js  | `opus` | stays NON-SKIPPABLE |
| `debug-cycle-2`             | gsd-t-debug.workflow.js   | `opus` | cycle-1 opus → cycle-2 fable → needs-human |
...
2. The 6 designated fable stage keys resolve to `opus`; competition producers resolve to `opus`.
...
| `pro` | red-team + pre-mortem + debug-cycle-2 | those 3 → opus; probes→opus, judge→sonnet; producers→opus |
| `premium` | all 6 (M85 full set) | all 6 designated stages → opus; producers→opus |
...
- `competition-producers` in stageOverrides: silently dropped; producers always resolve to `claude-opus-5`.
- `stageOverrides["competition-judge"]` resolving to `claude-opus-5` (producers' model): override dropped, profile tier used instead + `configError` marker.
...
2. `set-stage competition-judge <tier>` where `MODEL_IDS[tier] === "claude-opus-5"`: REJECTED — judge model must differ from producers'.
3. At resolve time: if a hand-edited config carries `stageOverrides["competition-judge"]` resolving to `claude-opus-5`, the override is dropped and a `configError` marker is emitted.
