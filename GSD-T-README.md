# GSD-T Framework Reference — v4.10.12
...
|---------|-----------|----------------|
| 4.10.12 | graph fix | Freshness hash-mismatch fix. The freshness checker hashed files with md5 but the indexer stores sha256 — a permanent mismatch that made EVERY file read as "changed" on every query, so the graph re-indexed the whole repo (re-running SCIP) just to answer a status check. `gsd-t graph status` on a large project went 30s-timeout-then-unavailable → 0.38s ok. Freshness exclude-dirs synced with the indexer's skip-dirs (no more phantom adds from `.venv`/`site-packages`). Unblocks cheap graph queries for the upcoming grep-intercept work. |
