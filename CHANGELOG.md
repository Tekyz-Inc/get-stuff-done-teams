...
**Known limitation (documented):** audit immutability is trigger-based defense-in-depth, NOT cryptographic tamper-proofing against a hostile process with direct .db-file write access (a fundamental SQLite limit). Tamper-evidence (hash-chain) is backlog #48. Built on branch `m100-define-trace-audit-logging` (behind main's 4.19.14) — merge to main is the next step.
